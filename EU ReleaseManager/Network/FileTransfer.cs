...
{
	public partial class FileTransfer : UserControl, IFileTransfer
	{
		private const int bufferSize = 10;
		private const int refreshInterval = 1000;

		public MyWebClient Client { get; private set; }
		...
		private void CleanUp()
		{
			...
			if (showProgress)
			{
				// stop display timer and disable GUI
				progressTimer.Stop();
				DisableProgress();
				showProgress = false;
			}
		...
		private void EnableProgress()
		{
			if (grpProgress.InvokeRequired)
				grpProgress.Invoke(new DelegateVoid(EnableProgress));
			else
				grpProgress.Enabled = true;
		}
		...
		{
			if (grpProgress.InvokeRequired)
				grpProgress.Invoke(new DelegateVoid(DisableProgress));
			else
				grpProgress.Enabled = false;
		}
